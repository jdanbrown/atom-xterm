...
const createColorsStyleSheet = function(parent, termID, colors) {
  const termHTMLID = `term3-term-${termID}`;
  const title = `${termHTMLID}-colors`;
  const ss = document.querySelector(`style[title=\"${title}\"]`);
  if (ss !== null) {
    ssEl.parentElement.removeChild(ssEl);
  }
  let styles = `\#${termHTMLID} { color: red; }`;
  const stylePrefix = `\#${termHTMLID}`;
  styles = [];
  ...
  return parent.appendChild(ssEl);
};
