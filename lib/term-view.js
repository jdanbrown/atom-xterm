...
class TermView {
  constructor(opts = { state: {} }) {
    this.uid = opts.state.uid || this.generateUid();
  ...
  getURI() {
    // TODO Include all of state = this.serialize()
    return `atom://xterm-term-view?uid=${this.uid}`;
  }
  ...
  serialize() {
    return {
      ...
      projectRootDirectoriesPaths: atom.project.rootDirectories.map(
        x => x.path,
      ),
    };
  }
  ...
  attached() {
    ...
    if (!this.opts.forkPTY) {
      ...
      this.ptyProcess = Task.once(
        processPath,
        // fs.normalize instead of fs.absolute so that symlinked paths are preserved
        fs.normalize(left != null ? left : '~'),
        shellOverride,
        cols,
        rows,
        args,
  ...
  paste() {
    // TODO Delegate to xterm handlers.Clipboard.pasteHandler so it can own this concern?
    let text = '';
    if (this.term.bracketedPasteMode) {
      text += '\x1b[200~';
    }
