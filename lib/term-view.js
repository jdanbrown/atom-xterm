...
const prepareTextForClipboard = require('xterm/lib/handlers/Clipboard')
...
class TermView {
  ...
  attached() {
    ...
    term.on('selection', ({ contents }) =>
      atom.clipboard.write(prepareTextForClipboard(contents)),
    );
  ...
  applyStyle() {
    ...
    return (this.term.element.style.fontSize = `${this.opts.fontSize ||
  ...
  attachEvents() {
    ...
    return this.disposable.add(
      atom.commands.add(this.getElement(), 'xterm:paste', () => this.paste()),
      atom.commands.add(this.getElement(), {
